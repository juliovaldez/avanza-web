...
@Component({
  selector: "app-form-txn-document-in",
  standalone: true,
  imports: [DxPopupModule, DxButtonModule, DxFormModule, DxDataGridModule],
  templateUrl: "./form-txn-document-in.component.html",
})
export class FormTxnDocumentInComponent {
  ...
  initRelatedData(): void {
    this.subscriptions.push(
      this.userStore
        ._load({
          filter: ["groups", "=", Group.PROVEDOR],
        })
        .subscribe({
          next: (data: any) => {
            this.fromUsersDatasource = data.data;
          },
        }),
      this.userStore
        ._load({ filter: ["groups", "=", Group.ALMACEN] })
        .subscribe({
          next: (data: any) => {
      ...
      this.txnTypesStore
        ._load({ filter: ["name", "=", TransactionType.IN] })
        .subscribe({
          next: (data: any) => {
