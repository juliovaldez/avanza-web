...
import { groups_list } from "@components/widget.types";
...
export class ListGroupsComponent
  implements LazyComponent<groups_list>, OnDestroy
{
  ...
  onToolbarPreparing(e: any) {
    ...
    e.toolbarOptions.items.forEach((item: any) => {
      if (item.name === "addRowButton") {
        item.options.onClick = () => this.loadForm(new Group());
      }
    });
  ...
  async loadForm(group: Group) {
    await this.lazyLoadService.load({
      component_name: lazyWidgets.group_form,
  ...
  async loadlistPermissions() {
    await this.lazyLoadService.load({
      component_name: lazyWidgets.permissions_list,
      container: this.lazyContainer,
      data: {
        model: null,
        config: { onPopup: true },
