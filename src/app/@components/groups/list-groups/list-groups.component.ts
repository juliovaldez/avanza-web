...
export class ListGroupsComponent implements OnDestroy {
  ...
  async loadForm(id: number = 0) {
    const instance: FormGroupComponent = await this.lazyLoadService.load(
  ...
  async loadlistPermissions() {
    const instance: ListPermissionsComponent = await this.lazyLoadService.load(
